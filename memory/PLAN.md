...
### 2026-04-14 Cloudflare Workers Migration

- The deployment target is being moved from Vercel-style hosting assumptions to Cloudflare Workers using `@opennextjs/cloudflare` and `wrangler`.
- The repo now needs first-class `wrangler.jsonc`, `open-next.config.ts`, and package scripts for `preview`, `deploy`, and Cloudflare env type generation.
- Vercel-specific runtime hooks should be removed or neutralized when they are not required for app behavior, while keeping product behavior unchanged.
- The main residual deployment risk is provider compatibility at runtime: MongoDB/Mongoose, NextAuth, SMTP, and any Node-heavy libraries must be validated in the Workers runtime after config migration.
